...
class NGCF(nn.Module):
    def __init__(self, config):
        ...
        self.layer_size = config.embed_size  
        self.n_layers = config.gcn_layers


        self.node_dropout = config.get('node_dropout', 0.1)  # 节点dropout
        self.mess_dropout = config.get('mess_dropout', 0.1)  # 消息dropout

        self.embedding_user = nn.Embedding(self.n_users, self.emb_size)
        self.embedding_item = nn.Embedding(self.n_items, self.emb_size)


        self.weight_dict = nn.ParameterDict()
        for k in range(self.n_layers):
            self.weight_dict.update({
                'W_gc_%d' % k: nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.layer_size, self.layer_size))),
                'b_gc_%d' % k: nn.Parameter(nn.init.zeros_(torch.empty(1, self.layer_size))),
                'W_bi_%d' % k: nn.Parameter(nn.init.xavier_uniform_(torch.empty(self.layer_size, self.layer_size))),
                'b_bi_%d' % k: nn.Parameter(nn.init.zeros_(torch.empty(1, self.layer_size)))
            })
        ...
        print(
            f"NGCF ready to go (layers:{self.n_layers}, node_drop:{self.node_dropout}, mess_drop:{self.mess_dropout})")

    def _set_graph(self, graph):

        self.Graph = graph.to(self.embedding_user.weight.device)
        self.Graph = self.Graph.to_sparse_csr()  # 或者 coo, 根据 pytorch 版本决定
    ...
    def computer(self):
        ...
        i_emb = self.embedding_item.weight
        ...
        for k in range(self.n_layers):
            ...
            side_emb = torch.sparse.mm(A_hat, all_emb)




            # 这里的 all_emb 是上一层的 ego_emb
            sum_emb = torch.matmul(side_emb + all_emb, self.weight_dict['W_gc_%d' % k]) + self.weight_dict[
                'b_gc_%d' % k]
