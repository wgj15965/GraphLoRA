...
def uAUC_pandas(user, predict, label):
    """
    Calculate UAUC (User-weighted Area Under the Curve) using Pandas for recommendation systems.

    Args:
        user (array-like): User IDs for each user-item pair.
        predict (array-like): Predicted scores for user-item pairs.
        label (array-like): Binary labels (1 for positive, 0 for negative).

    Returns:
        tuple: (uauc, computed_users, auc_values)
            - uauc (float): User-weighted AUC score.
            - computed_users (list): List of user IDs with valid AUC calculations.
            - auc_values (list): List of AUC scores for each computed user.
    """
    ...
    for user_id, group in grouped:
        ...
        user_scores = group['predict'].values

        # Skip if no positive or negative samples
        if not (np.any(user_labels == 1) and np.any(user_labels == 0)):
            continue

        # Calculate AUC using sklearn
        try:
            auc = roc_auc_score(user_labels, user_scores)
        except ValueError:
            continue

        # Weight by number of positive interactions
        weight = np.sum(user_labels == 1)

        computed_users.append(user_id)
        auc_values.append(auc)
        total_weighted_auc += auc * weight
        total_weight += weight

    # Calculate UAUC
    uauc = total_weighted_auc / total_weight if total_weight > 0 else 0.0

    return uauc, computed_users, auc_values
    ...
    def evaluation(self, model, data_loaders, cuda_enabled=True):
        ...
        for data_loader in data_loaders.loaders:
            ...
            if is_dist_avail_and_initialized():
                ...
                uauc, _, _ = uAUC_pandas(users_a,results_logits_a,labels_a)
                print("finished comput auc.....")
            else:
                auc = roc_auc_score(labels_.cpu().numpy(), results_logits_.cpu().numpy())
                uauc = uAUC_pandas(users_.cpu().numpy(), results_logits_.cpu().numpy(), labels_.cpu().numpy())
