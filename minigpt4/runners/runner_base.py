...
class RunnerBase:
    ...
    # nan不是每五轮导致的，所以这一版train是每五轮进行一次valid ,节省时间
    # def train(self):
    ...
    #
    #     eval_interval = self.config.run_cfg.get("eval_interval", 10)
    #     self.log_config()
    #
    ...
    #                     (cur_epoch + 1) % eval_interval == 0 or cur_epoch == self.max_epoch - 1
    #             ):
    #                 for split_name in self.valid_splits:
    #                     logging.info(f"正在验证 {split_name}.")
    #                     # 验证前清理显存
    #                     import torch
    ...
    #                         ), "验证日志中未找到 agg_metrics."
    #                         agg_metrics = val_log["agg_metrics"]
    #                         if agg_metrics > best_agg_metric and split_name == "valid":
    #                             best_epoch, best_agg_metric = cur_epoch, agg_metrics
    #                             self._save_checkpoint(cur_epoch, is_best=True)
    #                             logging.info(f"New best uAUC: {best_agg_metric} at epoch {cur_epoch}")
    #                             not_change = 0
    #                         else:
    #                             logging.info(f"uAUC: {agg_metrics}, not better than best: {best_agg_metric}")
    ...
    #                 break
    #             if not_change > 5:
    #                 logging.info("提前停止。连续5次验证无改进。")
    #                 break
    #
    #         if self.evaluate_only:
    #             print("训练完成或仅进行评估...")
    #             logging.info(f"正在验证 {self.test_splits[0]}.")
    ...
    #         logging.info(f"训练总耗时: {total_time_str}")
        ...
        if not self.evaluate_only:  # with training
            for cur_epoch in range(self.start_epoch, self.max_epoch):
                ...
                if not self.model_to_betrained():
                    break
                if not_change > 50:
                    logging.info("Early stop. The results has not changed up to 50 epochs.")
                    break
    ...
    def log_stats(self, stats, split_name):
        ...
        if isinstance(stats, dict):
            log_stats = {**{f"{split_name}_{k}": convert_to_serializable(v) for k, v in stats.items()}}

            # 写入原有的日志文件
            with open(os.path.join(self.output_dir, f"{split_name}_log.txt"), "a") as f:
                f.write(json.dumps(log_stats) + "\n")

            # 专门保存评估指标
            if split_name in self.valid_splits + self.test_splits:
                metrics_file = os.path.join(self.output_dir, f"{split_name}_metrics.json")

                # 提取关键指标
                metrics = {
                    "dataset": split_name,
                    "epoch": stats.get('epoch', 'unknown'),
                ...
                # 处理AUC
                if 'auc' in stats:
                    metrics["metrics"]["auc"] = convert_to_serializable(stats['auc'])

                # 处理uAUC - 根据你的输出格式，uauc是一个元组(分数, 用户列表)
                if 'uauc' in stats:
                    uauc_data = stats['uauc']
                    if isinstance(uauc_data, (list, tuple)) and len(uauc_data) >= 2:
                        metrics["metrics"]["uauc"] = {
                            "score": convert_to_serializable(uauc_data[0]),
                            "computed_users": convert_to_serializable(uauc_data[1])
                        }
                    else:
                        metrics["metrics"]["uauc"] = convert_to_serializable(uauc_data)

                # 添加其他可能的指标
                for key in ['acc', 'loss']:
                    if key in stats:
                        metrics["metrics"][key] = convert_to_serializable(stats[key])
